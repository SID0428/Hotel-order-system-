def restaurant():
    ...
    import csv
    import segno
    con=mysql.connector.connect(host='localhost',user='root',passwd="12345",database="siddharth")
    #if con.is_connected():
     #   print("Connection OK")
    #else:
     #   print("Error connecting to mysql database.")
    cur=con.cursor()
    #cur.execute("CREATE TABLE Customer_data(S_No INT auto_increment primary key,Name char(255), DOB date , Mobile int, Order_ varchar(255));")
                    ...
                    if abc==1:
                        ...
                        if dob==int(date) and ob==int(month):
                            ...
                            if pay==3:
                                ...
                                print("Please make your paymnet at the reception")
                            my_string = ', '.join(l)
                            cur.execute("INSERT INTO customer_data (Name, DOB, Mobile, Order_) VALUES (%s,%s,%s,%s);",(name,bday,mob,my_string))
                            con.commit()
                            rec=[name,bday,mob,*l]
                            write.writerow(rec)
                            fh.close()
